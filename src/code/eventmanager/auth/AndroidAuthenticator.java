/*
 * Copyright (C) 2011 Prasanta Paul
...
public class AndroidAuthenticator implements Authenticator {
	...
	public String getAuthToken(String service) {
		...
		if (acs == null || acs.length == 0) {
			Toast.makeText(context, "No Google Account Added...", Toast.LENGTH_LONG).show();
			return "";
		}
		...
		for (int i = 0; i < acs.length; i++) {
			if (acs[i].type.equals("com.google")) {
				...
				AccountManagerFuture<Bundle> result = (AccountManagerFuture<Bundle>) (manager.getAuthToken(acs[i], service, true, null, null));
